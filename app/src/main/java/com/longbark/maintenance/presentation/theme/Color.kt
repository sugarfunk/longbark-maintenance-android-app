package com.longbark.maintenance.presentation.theme

import androidx.compose.ui.graphics.Color

// Brand colors
val LongBarkPrimary = Color(0xFF2563EB) // Blue
val LongBarkSecondary = Color(0xFF10B981) // Green
val LongBarkTertiary = Color(0xFF8B5CF6) // Purple

// Status colors
val HealthyGreen = Color(0xFF10B981)
val WarningYellow = Color(0xFFF59E0B)
val CriticalRed = Color(0xFFEF4444)

// Light theme
val md_theme_light_primary = Color(0xFF2563EB)
val md_theme_light_onPrimary = Color(0xFFFFFFFF)
val md_theme_light_primaryContainer = Color(0xFFDDE9FF)
val md_theme_light_onPrimaryContainer = Color(0xFF001B3E)
val md_theme_light_secondary = Color(0xFF10B981)
val md_theme_light_onSecondary = Color(0xFFFFFFFF)
val md_theme_light_secondaryContainer = Color(0xFFD1F4E8)
val md_theme_light_onSecondaryContainer = Color(0xFF00201A)
val md_theme_light_tertiary = Color(0xFF8B5CF6)
val md_theme_light_onTertiary = Color(0xFFFFFFFF)
val md_theme_light_tertiaryContainer = Color(0xFFEDE7FF)
val md_theme_light_onTertiaryContainer = Color(0xFF1F004D)
val md_theme_light_error = Color(0xFFEF4444)
val md_theme_light_errorContainer = Color(0xFFFEE2E2)
val md_theme_light_onError = Color(0xFFFFFFFF)
val md_theme_light_onErrorContainer = Color(0xFF7F1D1D)
val md_theme_light_background = Color(0xFFF9FAFB)
val md_theme_light_onBackground = Color(0xFF111827)
val md_theme_light_surface = Color(0xFFFFFFFF)
val md_theme_light_onSurface = Color(0xFF111827)
val md_theme_light_surfaceVariant = Color(0xFFE5E7EB)
val md_theme_light_onSurfaceVariant = Color(0xFF6B7280)
val md_theme_light_outline = Color(0xFF9CA3AF)
val md_theme_light_inverseOnSurface = Color(0xFFF9FAFB)
val md_theme_light_inverseSurface = Color(0xFF1F2937)
val md_theme_light_inversePrimary = Color(0xFF93C5FD)

// Dark theme
val md_theme_dark_primary = Color(0xFF93C5FD)
val md_theme_dark_onPrimary = Color(0xFF003060)
val md_theme_dark_primaryContainer = Color(0xFF1E40AF)
val md_theme_dark_onPrimaryContainer = Color(0xFFDDE9FF)
val md_theme_dark_secondary = Color(0xFF6EE7B7)
val md_theme_dark_onSecondary = Color(0xFF00382E)
val md_theme_dark_secondaryContainer = Color(0xFF059669)
val md_theme_dark_onSecondaryContainer = Color(0xFFD1F4E8)
val md_theme_dark_tertiary = Color(0xFFD8B4FE)
val md_theme_dark_onTertiary = Color(0xFF3C0072)
val md_theme_dark_tertiaryContainer = Color(0xFF6D28D9)
val md_theme_dark_onTertiaryContainer = Color(0xFFEDE7FF)
val md_theme_dark_error = Color(0xFFFCA5A5)
val md_theme_dark_errorContainer = Color(0xFFDC2626)
val md_theme_dark_onError = Color(0xFF690005)
val md_theme_dark_onErrorContainer = Color(0xFFFEE2E2)
val md_theme_dark_background = Color(0xFF111827)
val md_theme_dark_onBackground = Color(0xFFF9FAFB)
val md_theme_dark_surface = Color(0xFF1F2937)
val md_theme_dark_onSurface = Color(0xFFF9FAFB)
val md_theme_dark_surfaceVariant = Color(0xFF374151)
val md_theme_dark_onSurfaceVariant = Color(0xFF9CA3AF)
val md_theme_dark_outline = Color(0xFF6B7280)
val md_theme_dark_inverseOnSurface = Color(0xFF111827)
val md_theme_dark_inverseSurface = Color(0xFFF9FAFB)
val md_theme_dark_inversePrimary = Color(0xFF2563EB)
